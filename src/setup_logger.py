...
def logger(func: Callable) -> Any:
    ...
    async def wrapper(*args, **kwargs) -> Any:
        ...
        def safe_repr(obj):
            if isinstance(obj, (UploadFile, bytes)):
                return log_bytes_info(obj)
            if isinstance(obj, Request):
                return f"<Request {obj.method} {obj.url}>"
            try:
                return str(obj)
            except Exception:
                return f"<unrepr {type(obj).__name__}>"

        args_info = [safe_repr(arg) for arg in args]
        kwargs_info = {k: safe_repr(v) for k, v in kwargs.items()}
